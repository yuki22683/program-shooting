...
function findUnityEditorPath(): string | null {
  const possiblePaths = [
...
function parseUnityYaml(content: string): any[] {
  const objects: any[] = [];
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    case "list_scripts": {
      ...
      const searchPattern = (args as any)?.searchPattern;
    ...
    case "read_script": {
      const scriptPath = (args as any).scriptPath;
    ...
    case "get_project_settings": {
      ...
      const settingFile = (args as any)?.settingFile;
    ...
    case "search_in_scripts": {
      ...
      const caseSensitive = (args as any).caseSensitive ?? false;
      const assetsPath = path.join(UNITY_PROJECT_PATH, "Assets");
      const scripts = getFilesRecursively(assetsPath, /\.cs$/);
    ...
    case "get_scene_hierarchy": {
      const scenePath = (args as any).scenePath;

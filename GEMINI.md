...
**重要**: 言語によらず値が共通なデータ（画像パスなど）は、個別の言語セクションではなく `"common"` セクションで管理すること。

1. **対象**: キー名が `_image` で終わるもの、または全言語で値が同一であるべき設定値。
2. **場所**: 各JSONファイルのルートにある `"common"` オブジェクト内。
3. **動作**: `LocalizationManager.cs` が読み込み時に `"common"` セクションの内容を全言語の辞書にマージする。

**利点**:
- ファイルサイズが削減される（21言語分の重複を排除）。
- 画像パスの修正が必要な際、1箇所の修正で全言語に反映される。
- 翻訳作業（`batch_translate.py` 等）の対象から外れるため、誤って画像パスが翻訳される事故を防げる。

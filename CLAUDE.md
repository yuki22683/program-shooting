...
**過去の失敗事例（2026年1月）**: `localizedText.json`を修正したが表示が変わらなかった。原因は、各プログラミング言語のレッスンデータが**別ファイル**に格納されており、そちらが`localizedText.json`を**上書き**していたため。

**レッスンデータの読み込み順序** (`LocalizationManager.cs`):
1. `localizedText.json` を読み込み
2. 以下のレッスンファイルを順番に読み込み、**上書きマージ**:
   - `pythonLessons.json`
   - `javascriptLessons.json`
   - `typescriptLessons.json`
   - `javaLessons.json`
   - `cLessons.json` ← C言語のスライドデータはここ
   - `cppLessons.json`
   - `csharpLessons.json`
   - `assemblyLessons.json`

**再発防止: レッスンデータ修正時の必須確認**:
1. **修正対象のファイルを特定**: `{言語}Lessons.json`が存在する場合、そちらを修正すること
2. `localizedText.json`を修正しても効果がない場合がある
3. `Grep`で対象キーを検索し、どのファイルに存在するか確認すること

**確認コマンド例**:
```
Grep: pattern="c_lesson1_ex1_slide2_content" path=Assets/Resources
```

### 必須: JSON修正後の構文検証

**過去の失敗事例（2026年1月）**: JSONファイルを修正した際に余計な`"`を残してしまい、JSON構文エラーが発生。結果、ファイル全体が読み込めなくなり、すべてのキーが表示されなくなった。

**再発防止: JSON修正後の必須確認**:
1. JSONファイルを修正した後は、構文エラーがないか確認すること
2. 特に文字列の編集時は、開始`"`と終了`"`の対応を確認すること
3. エスケープ文字（`\n`, `\"`, `\\`など）の扱いに注意すること

**JSONの構文エラーが発生すると、そのファイル全体が読み込めなくなる。部分的な問題ではなく、全面的な障害となるため、修正後の検証は義務である。**
...
**過去の失敗事例（2026年1月）**: 参考サイトのイラストを更新した際、イラストファイルをコピーしただけで、JSONファイル内の`_image`キーの値（パス）を同期しなかった。その結果、スライドで古いイラストが表示され続けた。

**原因分析**:
1. イラストファイルのコピーだけでは不十分
2. JSONファイル内の`_image`キーの値が参考サイトの`.ts`ファイルと異なっていた
3. 参考サイトとUnityプロジェクトでイラストパスが別々に管理されていることを理解していなかった

**スライドイラストのデータフロー**:
```
参考サイト（.tsファイル）              Unityプロジェクト
├── tutorialSlides[].image  ────→  {言語}Lessons.json の _image キー
│   例: "/illustrations/languages/c.png"
│
└── public/illustrations/   ────→  Assets/Resources/illustrations/
    （イラストファイル本体）
```
...
1. **イラストファイルをコピー**:
   ```
   参考サイト: public/illustrations/ → Unity: Assets/Resources/illustrations/
   ```

2. **JSONの`_image`キーを同期**:
   同期スクリプトを実行: `python scripts/sync_slide_images.py`

   このスクリプトは参考サイトの全`.ts`ファイルから`image`フィールドを抽出し、
   Unityの`{言語}Lessons.json`の`_image`キーを更新する

3. **`_image`キーの存在を検証**:
   同期スクリプト実行後、**全言語**の`_image`キーが追加されたか確認する：
   ```bash
   python -c "import json; f=open(r'Assets/Resources/{言語}Lessons.json','r',encoding='utf-8'); d=json.load(f); print([k for k in d['en'].keys() if '_image' in k][:5])"
   ```
   空のリスト`[]`が返る場合、`_image`キーが追加されていない。

4. **Unityで確認**:
   - Unityをリフレッシュ
   - Playモードでスライドを表示し、新しいイラストが表示されることを確認

**イラストファイルのコピーだけで終わりにしてはならない。JSONの`_image`キーの同期と検証は義務である。**

### 過去の失敗事例: `_image`キー未追加

**過去の失敗事例（2026年1月）**: イラストファイルをコピーし、同期スクリプトも実行したが、C言語の`cLessons.json`に`_image`キーが存在しなかった。結果、`SlideManager.cs`がフォールバック先の`SlideImages`フォルダから古い画像を読み込み続けた。

**原因分析**:
1. 同期スクリプト実行後に`_image`キーの存在を検証しなかった
2. スクリプトの出力（「追加: ...」メッセージ）を確認しなかった
3. 特定の言語だけPlayモードで確認し、問題の言語（C言語）を確認しなかった

**再発防止**:
1. 同期スクリプト実行後、出力メッセージで全言語が処理されたか確認する
2. 新しく追加した言語は特に、`_image`キーが存在するか個別に検証する
3. 「更新なし」と表示された言語も、`_image`キーが最初から存在するか確認する

### 重要: JSON構造に関する注意

**過去の失敗事例（2026年1月）**: `sync_slide_images.py`スクリプトが`_image`キーをJSONのルートレベルに追加してしまい、`LocalizationManager.cs`のJSON読み込みでエラーが発生した。

**原因分析**:
1. JSONの構造は`Dictionary<string, Dictionary<string, string>>`（言語コード→キー値ペア）
2. スクリプトが誤ってルートレベルに直接キーを追加した
3. エラー: `Could not cast or convert from System.String to System.Collections.Dictionary`

**正しいJSON構造**:
```json
{
    "en": {
        "python_lesson1_ex1_slide1_image": "/illustrations/languages/python.png"
    },
    "ja": {
        "python_lesson1_ex1_slide1_image": "/illustrations/languages/python.png"
    }
...
```json
{
    "en": { ... },
    "ja": { ... },
    "python_lesson1_ex1_slide1_image": "/illustrations/languages/python.png"  // ← ルートレベルは不可
}
```

**再発防止**:
1. JSONファイルを修正するスクリプトを書く際は、各言語セクション内にキーを追加すること
2. 修正後は必ず`read_console`でJSON読み込みエラーがないか確認
3. エラー発生時は`git checkout`でJSONファイルを復元してから修正

### 重要: コードブロック vs インラインコードの使い分け

**過去の失敗事例（2026年1月）**: スライド内で`#include <stdio.h>`や`int main() { ... }`がインラインコード（バッククォート1つ）として記述されていたため、独立したコードブロックGameObjectではなく、色付きテキストとして表示された。

**原因分析**:
1. 参考サイトのフォーマットをそのままコピーしていた
2. インラインコード（`` `code` ``）とコードブロック（` ```c code ``` `）の表示の違いを理解していなかった
3. 重要なコード構造が目立たなくなることを考慮していなかった

**使い分けルール**:

| 用途 | フォーマット | 表示結果 |
|------|-------------|---------|
| 説明文中の短い参照 | `` `printf` `` | 色付きインラインテキスト |
| 重要なコード構造・構文 | ` ```c\n#include <stdio.h>\n``` ` | 独立したコードブロックGameObject |
| 実行例・サンプルコード | ` ```c\nint main() { ... }\n``` ` | 独立したコードブロックGameObject |
...
**過去の失敗事例（2026年1月）**: スライドコンテンツで`**身近な例え：**`のようにアスタリスク2つで囲んで強調表示を使用していた。このMarkdown記法はVRアプリ内で正しく解釈されず、`**`がそのまま表示されてしまった。

**原因分析**:
1. 参考サイトやWeb記事のMarkdown記法をそのままコピーしていた
2. VRアプリ内のテキスト表示がMarkdownの`**太字**`記法に対応していないことを理解していなかった
3. 全20言語以上のレッスンファイルに同様の問題が存在していた
...
|------|---------|------|
| `**太字**` | 禁止 | VRアプリで解釈されない |
| `# 見出し` | OK | SlideManagerで対応済み |
| `` `コード` `` | OK | インラインコードとして表示 |
| ` ```言語 ``` ` | OK | コードブロックとして表示 |

**スライドコンテンツ作成時の確認事項**:
1. `**`で囲んだ強調表示を使用しないこと
2. 強調したい場合は見出し（`#`）を使用するか、そのまま記述する
3. レッスンデータ追加後、`Grep`で`\*\*`を検索し、不正な強調表示がないか確認する

**確認コマンド**:
```bash
# 強調表示パターンの検索（コード内の正当な**は除外）
Grep: pattern="\*\*[^\*\n]{4,}\*\*" path=Assets/Resources glob=*Lessons.json
```
